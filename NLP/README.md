...
- `bilstm_fixed.ipynb`, `bilstm_attention_bpe.ipynb`, `transformer_akkadian_english.ipynb`, and `transformer_akkadian_mlm_to_mt.ipynb` read `['transliteration', 'translation']` and rename `transliteration -> source`
...
- source vocab size: `37,305`
- target vocab size: `17,527`
...
- notebook now contains a completed training trace and final held-out test evaluation
- this is still a simpler whitespace-token baseline than the later BPE models, but it is now a recoverable completed run
...
### 2. `bilstm_attention_bpe.ipynb` — BiLSTM + Bahdanau attention + BPE
...
- val: `6,220`
- test: `6,220`
- after max-length filtering (`<= 80` tokens):
  - train: `37,325`
  - val: `4,687`
  - test: `4,653`
...
- notebook now contains a full completed run with final held-out test evaluation
- among the recurrent models in this folder, this is the strongest completed baseline
...
### 3. `transformer_akkadian_english.ipynb` + `transformer_akkadian_english_runner.py` — vanilla Transformer + BPE
...
### 4. `transformer_akkadian_mlm_to_mt.ipynb` — MLM pretraining + MT fine-tuning
